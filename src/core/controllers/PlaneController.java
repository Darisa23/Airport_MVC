...
public class PlaneController {
    ...
    // Validación del formato del ID: dos letras mayúsculas seguidas de cinco dígitos
    ...
    public static Response createPlane(String id, String brand, String model, int maxCapacity, String airline) {
        try {
            ...
            if (!isNonEmpty(brand) || !isNonEmpty(model) || !isNonEmpty(airline)) {
                return new Response("Brand, model, and airline must not be empty", Status.BAD_REQUEST);
            }
            if (maxCapacity <= 0) {
